...
class postUpdateProductVariant_DuplicateUPC_SKU_Test extends OrbitTestCase
{
    ...
    /**
     * This test should produce an error, since upc 'UPC-002' are belongs to
     * 'Celana Murah' Product.
     */
    public function testSaveProductUpdate_updateVariantUPC_KemejaMahal()
    {
        $productVariant1 = ProductVariant::where('upc', 'UPC-101')->first();

        // Object of first "Kemeja Mahal"
        $kemejaMahal1 = new stdClass();
        $kemejaMahal1->variant_id = $productVariant1->product_variant_id;
        $kemejaMahal1->upc = 'UPC-002';
        $kemejaMahal1->sku = NULL;
        $kemejaMahal1->price = NULL;

        // It containts array of product_attribute_value_id
        $kemejaMahal1->attribute_values = [1, NULL, NULL, NULL, NULL];
        ...
        $errorMessage = Lang::get('validation.orbit.exists.product.upc_code', ['upc' => $kemejaMahal1->upc]);
    ...
    /**
     * This test should produce an error, since sku 'SKU-002' are belongs to
     * 'Celana Murah' Product.
     */
    public function testSaveProductUpdate_updateVariantSKU_KemejaMahal()
    {
        $productVariant1 = ProductVariant::where('sku', 'SKU-101')->first();

        // Object of first "Kemeja Mahal"
        $kemejaMahal1 = new stdClass();
        $kemejaMahal1->variant_id = $productVariant1->product_variant_id;
        $kemejaMahal1->upc = NULL;
        $kemejaMahal1->sku = 'SKU-002';
        $kemejaMahal1->price = NULL;
        ...
        $response = json_decode($return);
        $errorMessage = Lang::get('validation.orbit.exists.product.sku_code', ['sku' => $kemejaMahal1->sku]);
        $this->assertSame(Status::INVALID_ARGUMENT, (int)$response->code);
        $this->assertSame('error', $response->status);
        $this->assertSame($errorMessage, $response->message);
    }
    ...
    /**
     * This test should produce an error, since upc 'UPC-102' are belongs to
     * Variant of 'Kemeja Mahal' Product.
     */
    public function testSaveProductUpdate_updateUPC_BelongsToKemejaMahalVariant()
    {
        // POST data
        $_POST['product_id'] = 2;
        $_POST['upc_code'] = 'UPC-102';
    ...
    /**
     * This test should produce an error, since upc 'SKU-102' are belongs to
     * Variant of 'Kemeja Mahal' Product.
     */
    public function testSaveProductUpdate_updateSKU_BelongsToKemejaMahalVariant()
    {
        // POST data
        $_POST['product_id'] = 2;
        $_POST['product_code'] = 'SKU-102';
     ...
     * Variant of 'Kemeja Mahal' Product.
     */
    public function testSaveProductNew_UPC_BelongsToKemejaMahalVariant()
    {
        // POST data
        // POST data
        $_POST['merchant_id'] = 1;
    ...
    /**
     * This test should produce an error, since sku 'SKU-102' are belongs to
     * Variant of 'Kemeja Mahal' Product.
     */
    public function testSaveProductNew_SKU_BelongsToKemejaMahalVariant()
    {
        // POST data
        $_POST['merchant_id'] = 1;
        $_POST['product_name'] = 'Sepatu Mahal';
        $_POST['status'] = 'active';
        $_POST['upc_code'] = 'UPC-919';
        $_POST['product_code'] = 'SKU-102';
