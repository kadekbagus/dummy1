<?php
/**
 * Unit testing for ProductAPIController::getSearchProduct() with additional
 * parameter `withParams` specified.
 *
...
class postNewProductVariant_DuplicateUPC_SKU_Test extends OrbitTestCase
{
    ...
    public function tearDown()
    {
        ...
        // Flag for `withParams` for product variant
    ...
    public function testGetProduct_ID1_VariantList_withoutWithParams()
    {
        ...
        $url = '/api/v1/product/search?' . http_build_query($_GET);
        ...
        $this->assertSame('Kemeja Mahal', $product->product_name);

        // Variant
        $this->assertSame(2, count($product->variants));
    }

    public function testGetProduct_ID1_VariantList_WithParamsExcludeDefaultVariant()
    {
        // Set the client API Keys
        $_GET['apikey'] = 'abc123';
        $_GET['apitimestamp'] = time();
        $_GET['product_id'] = [1];
        $_GET['with'] = ['variants'];
        $_GET['with_params']['variant.exclude_default'] = 'yes';
    ...
    public function testGetProduct_ID1_VariantList_WithParamsExcludeDefaultVariant_IncludeTransactionStatus()
    {
        // Set the client API Keys
        $_GET['apikey'] = 'abc123';
        $_GET['apitimestamp'] = time();
        ...
        $_GET['with'] = ['variants'];
        $_GET['with_params']['variant.exclude_default'] = 'yes';
        $_GET['with_params']['variant.include_transaction_status'] = 'yes';
        ...
        // Variant
        $this->assertSame(1, count($product->variants));
        $this->assertSame('yes', $product->variants[0]->has_transaction);
    }

    public function testGetProduct_ID2_VariantList_WithParamsExcludeDefaultVariant_IncludeTransactionStatus()
    {
        // Set the client API Keys
        $_GET['apikey'] = 'abc123';
        $_GET['apitimestamp'] = time();
        ...
        $this->assertSame(1, count($response->data->records));
        $product = $response->data->records[0];
        $this->assertSame('Celana Murah', $product->product_name);

        // Variant
        $this->assertSame(1, count($product->variants));
        $this->assertSame('no', $product->variants[0]->has_transaction);
    }
}
