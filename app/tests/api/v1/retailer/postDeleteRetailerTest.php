<?php
/**
 * Unit testing for RetailerAPIController::postDeleteRetailer() method.
 *
...
class postDeleteRetailerTest extends OrbitTestCase
{
    /**
    ...
    public function tearDown()
    {
        ...
        // test
        $events = array(
            'orbit.retailer.postdeleteretailer.before.auth',
            'orbit.retailer.postdeleteretailer.after.auth',
            'orbit.retailer.postdeleteretailer.before.authz',
            'orbit.retailer.postdeleteretailer.authz.notallowed',
            'orbit.retailer.postdeleteretailer.after.authz',
            'orbit.retailer.postdeleteretailer.before.validation',
            'orbit.retailer.postdeleteretailer.after.validation',
            'orbit.retailer.postdeleteretailer.before.save',
            'orbit.retailer.postdeleteretailer.after.save',
            'orbit.retailer.postdeleteretailer.after.commit',
            'orbit.retailer.postdeleteretailer.access.forbidden',
            'orbit.retailer.postdeleteretailer.invalid.arguments',
            'orbit.retailer.postdeleteretailer.general.exception',
            'orbit.retailer.postdeleteretailer.before.render'
        );
    ...
    public function testNoAuthData_POST_api_v1_retailer_deleted()
    {
        $url = '/api/v1/retailer/delete';
    ...
    public function testInvalidSignature_POST_api_v1_retailer_delete()
    {
    ...
    public function testSignatureExpire_POST_api_v1_retailer_delete()
    {
        // Set the client API Keys
        $_GET['apikey'] = 'cde345';
    ...
    public function testAccessForbidden_POST_api_v1_retailer_delete()
    {
        // Set the client API Keys
        $_GET['apikey'] = 'cde345';
        $_GET['apitimestamp'] = time();
    ...
    public function testMissingRetailerId_POST_api_v1_retailer_delete()
    {
        // Set the client API Keys
        $_GET['apikey'] = 'cde345';
        $_GET['apitimestamp'] = time();
        ...
        $permission->permission_name = 'delete_retailer';
        $permission->save();

        $chuck->permissions()->attach($permission->permission_id, array('allowed' => 'yes'));

        $message = Lang::get('validation.required', array('attribute' => 'retailer id'));
        $data = new stdclass();
    ...
    public function testRetailerIdNotExists_POST_api_v1_retailer_delete()
    {
        // Data to be post
        $_POST['retailer_id'] = '99';
        ...
        $message = Lang::get('validation.orbit.empty.retailer');
        $data = new stdclass();
        $data->code = Status::INVALID_ARGUMENT;
        $data->status = 'error';
        $data->message = $message;
    ...
    public function testRetailerIdNotExists_StatusAlreadyDeleted_POST_api_v1_retailer_delete()
    {
        // Data to be post
        $_POST['retailer_id'] = '2';
    ...
    public function testMissingPassword_POST_api_v1_retailer_delete()
    {
        // Set the client API Keys
        $_GET['apikey'] = 'cde345';
        $_GET['apitimestamp'] = time();
    ...
    public function testWrongPasswordGiven_POST_api_v1_retailer_delete()
    {
        // Set the client API Keys
        $_GET['apikey'] = 'cde345';
        $_GET['apitimestamp'] = time();
    ...
    public function testReqOK_POST_api_v1_retailer_delete()
    {
        // Number of retailer not deleted before this operation
        ...
        $expect->message = Lang::get('statuses.orbit.deleted.retailer');
        $expect->data = NULL;
        $json_expect = json_encode($expect);

        $this->assertSame($json_expect, $return);

        // Fresh query the 'Alfa Gwalk'
        $retailer = Retailer::find(21);
        $this->assertSame('deleted', $retailer->status);

        $numAfter = Retailer::excludeDeleted()->count();
        $this->assertSame($numBefore - 1, $numAfter);
    }

    public function testSavedThenRollback_POST_api_v1_retailer_delete()
    {
        // Register an event on 'orbit.retailer.postdeleteretailer.after.save'
        // and thrown some exception so the data that has been saved
        // does not commited
        Event::listen('orbit.retailer.postdeleteretailer.after.save', function($controller, $retailer)
        {
